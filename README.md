...
<head>
    <meta charset="UTF-8">
    <title>Dashboard IoT Final Durasi</title>
...
<body>
    ...
    <script>
        ...
        function updateJam() {
            let now = new Date();
            let tgl = now.toLocaleDateString('id-ID', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' });
            let jam = now.toLocaleTimeString('id-ID');
            document.getElementById("datetime").innerHTML = tgl + "<br>" + jam;
        }
        ...
        // ===== STATUS =====
        let statusPerangkat = {
            lampu: "OFF", kipas: "OFF", pendingin: "OFF", pestida: "OFF"
        };

        let waktuMulai = {
            lampu: null, kipas: null, pendingin: null, pestida: null
        };
        ...
        function toggle(btn) {
            if (btn.classList.contains("off")) {
                ...
                if (btn.id === "pestida") {
                    alert("⚠️ BAHAYA! Pompa pestida Aktif! jangan memasuki greenhouse");
                }
            } else {
        ...
        window.onload = function () {
            ...
            function updateData() {
                ...
                card.style.backgroundColor = suhu > 30 ? "red" : suhu < 30 ? "lightblue" : "white";
